...
<script>
	...
	export default {
		...
		methods: {
			...
			// 		alert("用户权限不能为空")
			// 	} else {
			// 	// var dialog_info = new URLSearchParams()
			// 	// dialog_info.append("id", this.form.id)
			// 	// dialog_info.append("userName", this.form.userName)
			// 	// dialog_info.append("passWord", this.form.passWord)
			// 	// dialog_info.append("roleId", this.form.roleId)
			// 	axios.get('api/user/saveUser').then((response) => {
			...
			// 		})
			// 		} else {
			// 			alert("新增用户失败的原因：" + response.data.message)
			// 			}
			// 		}).catch((error) => {
			...
			submit() {
				...
				AddInfo(resquestData).then(response => {
